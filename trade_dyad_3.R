...
cov_ergm1$covar[cov_ergm1$covar<2007]<-1
cov_ergm1$covar[cov_ergm1$covar>1]<-0

##
library(statnet)
net_adjmat2 <- as.network(x = adjmat, # the network object
...
set.vertex.attribute(net_adjmat2,"covar", cov_ergm1$covar)
#set.edge.value(net_adjmat,"Weight",net_adjmat[,3])

# summarize
summary.network(net_adjmat2,print.adj = FALSE)

# plot the network
plot.network(net_adjmat2,
             vertex.col = "purple", #just one color
             displaylabels = F) # no node names

#edge.lwd = log(get.edge.value(net_adjmat,"Weight"))) # edge width

plot(net_adjmat2,displaylabels=T,label.cex=.5,edge.col=rgb(150,150,150,100,maxColorValue=255))
